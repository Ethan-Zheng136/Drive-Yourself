...
def transform_to_global(
    ...
    # dtype follows pos_local (codebook float32), not the ambient default dtype:
    # under Lightning bf16-true the default dtype is bf16, which would otherwise make
    # rot_mat bf16 and clash with the float32 codebook trajectory in bmm.
    rot_mat = torch.zeros((head_now.shape[0], 2, 2), device=head_now.device, dtype=pos_local.dtype)
    rot_mat[:, 0, 0] = cos
    rot_mat[:, 0, 1] = sin
    rot_mat[:, 1, 0] = -sin
    rot_mat[:, 1, 1] = cos
...
class ActionTokenizer:
    ...
    def rollout(self, action_tokens: torch.Tensor, time_steps: int) -> torch.Tensor:
        # initial state (match codebook dtype so bf16-true default dtype can't introduce a mismatch)
        pos_a = torch.tensor([[[0, 0]]], dtype=action_tokens.dtype) # [1, 1, 2]
        head_a = torch.tensor([[0]], dtype=action_tokens.dtype) # [1, 1]

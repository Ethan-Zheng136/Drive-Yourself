...
class AutoVLAAgent(AbstractAgent):
    ...
    def initialize(self) -> None:
        ...
        # --- YouDrive: no-merge persona vectors. Attach LoRA adapter(s) on top of the loaded base
        #     and combine by weights (task arithmetic). Controlled via env so the official eval
        #     pipeline needs no changes beyond setting these vars.
        import os as _os
        _aspec = _os.environ.get("PERSONA_ADAPTERS")
        if _aspec:
            from peft import PeftModel as _PeftModel
            _paths = [p for p in _aspec.split(",") if p]
            _weights = [float(w) for w in _os.environ.get("PERSONA_WEIGHTS", "1").split(",")]
            assert len(_weights) == len(_paths), "PERSONA_WEIGHTS must match PERSONA_ADAPTERS"
            self.autovla.vlm = _PeftModel.from_pretrained(self.autovla.vlm, _paths[0], adapter_name="a0")
            for _i, _p in enumerate(_paths[1:], 1):
                self.autovla.vlm.load_adapter(_p, adapter_name=f"a{_i}")
            _names = [f"a{_i}" for _i in range(len(_paths))]
            # YouDrive composable / no-merge LoRA: do NOT fuse adapters into a single vector.
            # add_weighted_adapter(combination_type="linear") requires EQUAL rank AND equal
            # target modules and crashes ("All adapters must have the same r value") on a
            # mixed stage1(r64, q/k/v/o+MLP) + stage2(r16, q/k/v/o) stack. Instead we scale
            # EACH adapter's contribution by its own weight via the per-adapter `scaling` dict
            # (keyed by adapter_name = the LoRA alpha/r gain) and activate ALL adapters at once.
            # PEFT applies multiple active adapters ADDITIVELY, which is rank-agnostic and does
            # NOT require equal rank/targets. This covers the single-adapter case identically
            # (one name, one weight -> set_adapter(["a0"])).
            _name2w = dict(zip(_names, _weights))
            for _, _mod in self.autovla.vlm.named_modules():
                if hasattr(_mod, "scaling") and isinstance(getattr(_mod, "scaling"), dict):
                    for _adp in list(_mod.scaling.keys()):
                        if _adp in _name2w:
                            _mod.scaling[_adp] = _mod.scaling[_adp] * _name2w[_adp]
            # NB: activate via the tuner (base_model). PeftModel.set_adapter() takes a single
            # str and rejects a list; base_model.set_adapter([...]) activates ALL of them.
            self.autovla.vlm.base_model.set_adapter(_names)
            self.autovla.vlm = self.autovla.vlm.to(self.autovla.device)
            self.autovla.eval()
            print(f"[persona] attached {_paths} weights={_weights}", flush=True)

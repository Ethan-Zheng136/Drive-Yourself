"""Stage-2 style cache for the NAVTEST split (precompute once, CPU).

NON-DESTRUCTIVE clone of youdrive/build_style_cache.py. The ONLY functional change vs the
original is that the split subdir (navsim_logs/<split>, sensor_blobs/<split>) is read from the
SPLIT env (default "test") instead of the original's hardcoded "trainval". Everything else
(feature math kin/interact, resample_teacher, norm stats) is byte-for-byte identical, so the
navtest cache is directly mergeable with the navtrain cache.

Run (navtest logs/sensors live under OPENSCENE_DATA_ROOT/{navsim_logs,sensor_blobs}/test):
  OPENSCENE_DATA_ROOT=/root/workspace/closed_loop/data/navsim \
  DDV2_JSON=/mnt/pfs/zhengguantian/autovla/persona/ddv2_navtest.json \
  OUT=/mnt/pfs/zhengguantian/autovla/persona/style_cache_ddv2_navtest.pkl \
  python youdrive/build_style_cache_navtest.py
"""
...
DATA = os.environ.get("OPENSCENE_DATA_ROOT", "/root/workspace/closed_loop/data/navsim")
SPLIT = os.environ.get("SPLIT", "test")
DDV2_JSON = os.environ.get("DDV2_JSON", "/mnt/pfs/zhengguantian/autovla/persona/ddv2_navtest.json")
OUT = os.environ.get("OUT", "/mnt/pfs/zhengguantian/autovla/persona/style_cache_ddv2_navtest.pkl")
DT = 0.5
CORRIDOR = 1.5
...
def resample_teacher(xy, meta):
    ...
    accel are on the SAME time base as DDv2 and the student rollout. Identical to the navtrain
    builder."""
...
def main():
    ...
    META = ddv2.get("_meta", {})
    print(f"[build_style_cache_navtest] split={SPLIT} teacher _meta={META}", flush=True)

#!/usr/bin/env bash
# Hydra-MDP full dump over 4049 styletest tokens, 8 GPUs -> compare_full/hydra_mdp.json
# Usage: bash dump_hydra_mdp.sh   (override GPUs: GPUS="0 1 2 3" bash dump_hydra_mdp.sh)
set -uo pipefail
...
export OUT_PATH=/mnt/pfs/zhengguantian/autovla/compare_full/hydra_mdp.json
...
for idx in "${!GPUS[@]}"; do
  CUDA_VISIBLE_DEVICES=${GPUS[$idx]} SHARD_IDX=$idx NUM_SHARDS=$N BATCH_SIZE=4 \
    "$PY" youdrive_dump_hydra.py > "$LOG/hydra_shard${idx}.log" 2>&1 &
  pids+=($!); echo "hydra shard $idx -> GPU ${GPUS[$idx]} (pid ${pids[-1]})"
done
for p in "${pids[@]}"; do wait "$p"; done
MERGE=1 "$PY" youdrive_dump_hydra.py
echo "DONE hydra_mdp -> /mnt/pfs/zhengguantian/autovla/compare_full/hydra_mdp.json"

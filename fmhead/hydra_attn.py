"""hydra_attn.py -- VENDORED verbatim from Hydra-MDP / GTRS.

Source: gtrs/navsim/agents/utils/attn.py (GTRS repo, the authentic Hydra-MDP baseline).
Copied here so FMHeadScorer can use Hydra-MDP's EXACT `MemoryEffTransformer` self-attention
block (same source) WITHOUT importing the gtrs package at runtime. Only whitespace/import
layout may differ; the classes/functions are unchanged.
"""
...
def summarize_qkv_chunk(q, k, v, mask, attn_bias_chunk, causal, qk_start_indices, dropout):
    q_start_index, k_start_index, q_chunk_size, k_chunk_size, device = \
        *qk_start_indices, q.shape[-2], k.shape[-2], q.device
    ...
    for q_index, q_chunk in enumerate(q_chunks):
        ...
        for k_index, (k_chunk, v_chunk, mask_chunk) in enumerate(zip(k_chunks, v_chunks, mask_chunks)):
            ...
            exp_weight_chunk, weighted_value_chunk, weight_max_chunk = summarize_qkv_fn(
                q_chunk, k_chunk, v_chunk, mask_chunk, attn_bias_chunk, causal,
                (q_start_index, k_start_index), dropout if training else 0.)
...
class Attention(nn.Module):
    def __init__(self, *, dim, heads=8, dim_head=64, dropout=0., causal=False,
                 memory_efficient=False, q_bucket_size=512, k_bucket_size=1024):
        super().__init__()
        self.heads = heads
        self.causal = causal
    ...
    def forward(self, q, k, v, mask=None, attn_bias=None, memory_efficient=None,
                q_bucket_size=None, k_bucket_size=None):
...
class MemoryEffTransformer(nn.Module):
    # `memory_efficient` toggles Attention's chunked+checkpointed branch. Hydra-MDP uses it
    # (True) for its 16384-entry vocab self-attn; for a handful of FMHead candidates (N<=16)
    # the plain branch (False) is identical math but faster and avoids checkpoint overhead.
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation=F.relu, layer_norm_eps=1e-5, memory_efficient=True):
        super().__init__()
        dim_head = d_model // nhead
        self.self_attn = Attention(dim=d_model, heads=nhead, dim_head=dim_head,
    ...
    def forward(self, x, need_mean=False):
        ...
        if need_mean:
            num_query, embed_dims, bs, num_bev_queue = (q.shape[1], q.shape[2],
                                                        q.shape[0] // 2, 2)
